  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8084/api/addNoticeInfo",
        formData,
        {
          headers: {
  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8084/api/editNotice/${noticeId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8084/api/removeNotice/${noticeId}`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
            "Content-Type": "application/json",
  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8084/api/getAllNotices`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
            "Content-Type": "application/json",
      ...
      dispatch({
        ...
        payload: "Failed to get Notices",
      });
    }
  };
}
export { addNotice, updateNotice, deleteNotice, getAllNotices };
