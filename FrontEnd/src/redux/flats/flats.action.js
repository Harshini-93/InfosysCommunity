  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8082/api/getAllFlats`,
        {
          headers: {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to get flats";
...
export function getAllResidents() {
  return async function (dispatch) {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to get residents";
...
export function addFlat(formData) {
  return async function (dispatch) {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to add flat";
...
export function addParking(formData) {
  return async function (dispatch) {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to add parking";
