  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:8083/api/addComplaint`,
        complaintData.data,
        {
          headers: {
      ...
      dispatch({
        ...
        payload: "Complaint Registered Sucessfully",
      });
    } catch (error) {
      const errorMessage =
        error.response?.data?.message || "unable to register complaint";
  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8083/api/closeComplaint/${complaintId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
      ...
      dispatch({
        ...
        payload: "Complaint Closed Successfully",
      });
    } catch (error) {
      const errorMessage =
        error.response?.data?.message || "Unable To Close Complaint";
  ...
  return async function (dispatch) {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8083/api/getAllComplaints`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
            "Content-Type": "application/json",
...
export { registerComplaint, closeComplaint, getAllComplaints };
